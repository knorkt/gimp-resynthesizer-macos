...
**3. Install MacPorts**
Download and install the `.pkg` for your macOS version from [MacPorts.org](https://www.macports.org/install.php), then update it:
...
This master script copies the frontend/backend, fixes the `rpath` glitch, severs MacPorts dependencies, and clears Gatekeeper quarantine restrictions.

Run this block directly from the `resynthesizer3` folder:
...
> **Heal Transparency** may throw an `Unknown Error` if executed on a flat layer without an active Alpha Channel.

---

## ⚠️ Portability & Troubleshooting (The `dylib` Danger Zone)

> [!WARNING]
> These compiled binaries are highly custom and **not universally portable**. They require an Apple Silicon (ARM64) architecture, an unmodified `/Applications/GIMP.app` directory path, and compatible GIMP internal library versions.

### Common Errors Matrix

| Status | Error Signature | Root Cause | The Fix |
| --- | --- | --- | --- |
| 🔴 | `dyld: Library not loaded` | GIMP moved its internal libraries, or the GIMP application was renamed. | Use `otool -L [engine_path]` to locate the break, then map it with `install_name_tool -change`. |
...
| 🟡 | `Unknown Error` (in GIMP GUI) | The C engine silently crashed in the background. | Launch GIMP via Terminal to read the raw crash log. |
| 🔵 | `Killed: 9` (in Terminal) | macOS Gatekeeper blocked execution (common when transferring `.zip` backups). | Run `sudo xattr -cr` and re-run the `codesign` command. |
...
**Launch GIMP via Terminal (to view silent crash logs):**
...
**Clear Gatekeeper quarantine globally for GIMP:**
